        ...
        try:
            with open(command_dict_filepath) as fobj:
                trappedbot.LOGGER.debug(
                    f"Loading command dictionary at {command_dict_filepath}"
                )
            ...
            if "paths" in self.command_dict.keys():
                os.environ["PATH"] = os.pathsep.join(
                    self.command_dict["paths"] + [os.environ["PATH"]]
                )
                trappedbot.LOGGER.debug(f'Path modified. Now: {os.environ["PATH"]}.')

        except FileNotFoundError:
            trappedbot.LOGGER.error(f"File not found: {command_dict_filepath}")
    ...
    def assert_sanity(self):
        ...
        for key in self.command_dict.keys():
            if key not in ("commands", "paths"):
                raise CommandDictSanityError(f"Invalid toplevel child found: {key}.")
        ...
        if "paths" in self.command_dict:
            if type(self.command_dict["paths"]) != list:
                raise CommandDictSanityError('The "paths" node must be a list.')
            ...
            for com in self.command_dict["commands"]:
                ...
                for opt in self.command_dict["commands"][com].keys():
                    ...
                    if opt not in (
                        ...
                        raise CommandDictSanityError(
                            f'In command "{com}", invalid option found: ' f'"{opt}".'
                        )
                    # Implement rule 6
    ...
    def match(self, string):
        ...
        if string in self.commands.keys():
            ...
            for command in self.commands.keys():
                if "regex" in self.commands[command].keys() and re.match(
                    self.commands[command]["regex"], string
                ):
                    matched = True
                    cmd = command
                    break
